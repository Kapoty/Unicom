...
import { useOrigemDeleteMutation, useOrigemPatchMutation, useOrigemPostMutation } from "./OrigemMutations";
import { OrigemPatchRequestSchema, OrigemPostRequestSchema } from "./OrigemPayloads";
import { useOrigemAdminByEmpresaIdAndOrigemIdQuery } from "./OrigemQueries";

const OrigemFormSchema = z.object({
...
type OrigemFormData = z.infer<typeof OrigemFormSchema>;

const OrigemFormPage = () => {

	const { origemId: origemIdParam } = useParams();
	const origemId = parseInt(origemIdParam!);

	const empresaId = useEmpresaIdParam();

	const isEditMode = origemIdParam !== 'add';

	const [origem, setOrigem] = useState<IOrigemAdmin>();
	...
	const { data, refetch, isFetching, error } = useOrigemAdminByEmpresaIdAndOrigemIdQuery(empresaId, isEditMode ? origemId : undefined);

	const { mutateAsync: postOrigem } = useOrigemPostMutation();
	const { mutateAsync: patchOrigem } = useOrigemPatchMutation();
	const { mutateAsync: deleteOrigem } = useOrigemDeleteMutation();
	...
	const {
		...
		reset,
		control
	} = useForm<OrigemFormData>({
		defaultValues: {
			nome: '',
			createdAt: null,
			updatedAt: null,
		},
		resolver: zodResolver(OrigemFormSchema),
		mode: 'onChange',
		disabled: isUpdating,
	});
	...
	const onSubmit = async (data: OrigemFormData) => {
		try {
			if (isEditMode)
				await patch(data)
			else
	...
	const patch = async (data: OrigemFormData) => {
		const response = await patchOrigem({
			empresaId: empresaId!,
			origemId: origemId!,
			payload: OrigemPatchRequestSchema.parse({
				...data,
			}),
		});
		reset();
	...
	const post = async (data: OrigemFormData) => {
		const response = await postOrigem({
			empresaId: empresaId!,
			payload: OrigemPostRequestSchema.parse({
				...data,
			}),
		});
		unblockNavigation?.();
		browserHistory.push(`/e/${empresaId}/cadastros/origens/${response.origemId}`);
	}

	const _delete = async () => {
		try {
			const response = await deleteOrigem({
				empresaId: empresaId!,
				origemId: origemId!,
			});
			enqueueSnackbar('Excluído com sucesso!', { variant: 'success' });
			unblockNavigation?.();
			browserHistory.push(`/e/${empresaId}/cadastros/origens`);
		} catch (error) {
	...
	return <DashboardContent
		titulo={isEditMode ? 'Editar Origem' : 'Nova Origem'}
		fabs={[
			<CustomFab tooltip={{ title: 'Voltar' }} key={0} onClick={() => browserHistory.push(`/e/${empresaId}/cadastros/origens`)} ><ArrowBack /></CustomFab>,
			...formFabs
		]}
	>
		{(!origem && isFetching) ? <Carregando /> :
			(!origem && error) ? <Alert severity='error'> Falha ao carregar </Alert> :
