...
import { GridPanelProps, PanelPropsOverrides, useGridApiContext } from "@mui/x-data-grid-premium";
...
const CustomPanel = (props: GridPanelProps & PanelPropsOverrides) => {
	...
	if (!isMobile)
		return <Popover
			open
			onClose={() => props.onClose?.() ?? apiRef.current.hidePreferences()}
			anchorEl={apiRef.current.rootElementRef.current?.querySelector(".MuiDataGrid-main")}
			container={apiRef.current.rootElementRef.current}
		>
			{props.children}
		</Popover>
