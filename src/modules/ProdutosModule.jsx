...
class ProdutosModule extends React.Component {
	...
	constructor(props) {
		...
		this.columns = [
			...
			{ field: "actions", headerName: "Ações", minWidth: 100, renderHeaderFilter: () => "", renderCell: (params) => <Stack direction="row" spacing={1}>
				<Tooltip title="Editar" onClick={() => this.props.navigate("/empresa/produtos/" + params.row.produtoId)}>
	...
	getProdutoListFromApi() {
		...
		api.get("/empresa/me/produto")
			.then((response) => {
				let produtoRows = response.data.map((produto) => {return {
				...
				this.setState({produtoList: response.data, produtoRows: produtoRows, calling: false});
			})
			.catch((err) => {
				console.log(err);
				...
				<Paper elevation={0} sx={{flexGrow: 1, padding: 2, minHeight: "100%", minWidth: "800px", boxSizing: "border-box", display: "flex", flexDirection: "column", aligmItems: "center", justifyContent: "start"}} className="modulePaper">
					<Stack direction="row" gap={3} alignItems="center">
						...
						<ButtonGroup sx={{marginBottom: 3}}>
								<Button variant="contained" size="large" startIcon={<NoteAddIcon />} onClick={() => this.props.navigate("/empresa/produtos/novo")}>Novo Produto</Button>
						</ButtonGroup>
					</Stack>
					<Box sx={{ flexGrow: 1, height: "1px", minHeight: "400px" }}>
						<CustomDataGridPremium
							rows={this.state.produtoRows}
							...
							loading={this.state.produtoList == null || this.state.calling}
							sx={{marginBottom: 1}}
							pagination
							headerFilters
							disableAggregation
...
export default (props) => {
	...
	return <ProdutosModule params={params} location={location} navigate={navigate} {...props}/>
}